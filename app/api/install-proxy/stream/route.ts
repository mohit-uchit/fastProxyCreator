...
const sendLog = (installationId: string, message: string) => {
  ...
  if (controller) {
    try {
      ...
      const logMessage = `data: ${JSON.stringify({ type: 'log', message })}\n\n`
      controller.enqueue(encoder.encode(logMessage))
    } catch (error) {
      console.error(`SSE: Error sending log for installation ${installationId}:`, error)
    }
  }
}
    ...
    setupSquidProxy({
      ...
      if (controller) {
        try {
          ...
          console.error(`SSE: Error sending completion for installation ${installationId}:`, error)
        }
        activeInstallations.delete(installationId)
      }
    }).catch((error) => {
      ...
      if (controller) {
        try {
          ...
          console.error(`SSE: Error sending error for installation ${installationId}:`, error)
        }
        activeInstallations.delete(installationId)
      }
    })
